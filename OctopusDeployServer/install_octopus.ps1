...
Function Register-OctopusTentacle([string]$EnvironmentName, [string]$AppRole, [string]$OctopusServerThumbprint, [string]$OctopusAPIKey, [string]$Localhostname){
    ...
    $TentacleRegistrator = 'register-with --instance "Tentacle" --server "http://octopus.eu-west-1.coolblue-shared.eu" --apiKey={0} --role "{1}" --environment "{2}" --comms-style TentaclePassive --name="{3}" --console --force --publicHostName="{3}"' -f $OctopusAPIKey, $AppRole, $Environmentname, $localhostname
...
# & osql -E -S .\SQLEXPRESS -i C:\vagrant\enable_sa.sql
